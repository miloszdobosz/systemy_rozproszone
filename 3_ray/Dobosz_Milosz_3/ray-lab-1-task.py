...
@ray.remote
def bubble_sort_distributed(sequence):
    return bubble_sort(sequence)


# Normal Python in a single process
def run_local_bubble_sort(sequence):
    results = [bubble_sort(sequence) for _ in range(os.cpu_count())]
    return results


from random import randint

sequence = [randint(0, 1000) for _ in range(1000)]

print("local bubble sort")
cProfile.run("run_local_bubble_sort(sequence)")

# Distributed on a Ray cluster
def run_remote_bubble_sort(sequence):
    results = ray.get(
        [bubble_sort_distributed.remote(sequence) for _ in range(os.cpu_count())]
    )
    return results


print("remote bubble sort")
cProfile.run("run_remote_bubble_sort(sequence)")
