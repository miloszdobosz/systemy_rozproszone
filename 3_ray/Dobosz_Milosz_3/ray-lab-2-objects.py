...
large_lists = ([np.random.randint(100000) for _ in size] for _ in count)
large_dicts = ({i: np.random.randint(100000) for i in size} for _ in count)

print("put lists")
obj_ref_lists = ray.put([l for l in large_lists])

print("put dicts")
obj_ref_dicts = ray.put([d for d in large_dicts])


@ray.remote
def process_data(lists, dicts):
    lists = ray.get(lists)
    dicts = ray.get(dicts)

    lists = [sorted(l) for l in lists]
    dicts = [sorted(d) for d in dicts]

    ray.put([l for l in lists])
    ray.put([d for d in dicts])

print("run process data")
process_data.remote(obj_ref_lists, obj_ref_dicts)
